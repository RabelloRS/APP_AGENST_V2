...
def show_crews_tab():
    ...
    with st.expander("🔄 Sistema de Sincronização Automática", expanded=False):
        ...
        try:
            ...
            with col1:
                st.metric("📊 Status do Sistema", "✅ Ativo" if sync_status.get("sync_available") else "❌ Inativo")
            ...
            with col2:
                ...
                st.metric(
                    "🔄 Sincronização Automática", "✅ Ativa" if sync_status.get("auto_sync_enabled") else "❌ Inativa"
                )
            ...
            if st.button("🔄 Executar Sincronização Manual", type="secondary"):
                with st.spinner("Executando sincronização..."):
                    ...
                    if sync_result.get("status") == "completed":
                        st.success(
                            f"✅ Sincronização concluída! {sync_result.get('crews_checked', 0)} crew(s) verificada(s)"
                        )
                    ...
                    with col_controls:
                        ...
                        with col_up:
                            if st.button("⬆️", key=f"up_{i}", help="Mover para cima", disabled=i == 0):
                                if i > 0:
                                    (
                                        st.session_state.selected_crew_agents[i],
                                        st.session_state.selected_crew_agents[i - 1],
                                    ) = (
                                        st.session_state.selected_crew_agents[i - 1],
                                        st.session_state.selected_crew_agents[i],
                                    )
                        ...
                        with col_down:
                            if st.button(
                                ...
                                disabled=i == len(st.session_state.selected_crew_agents) - 1,
                            ):
                    ...
                    if not is_selected:
                        if st.button("➕ Adicionar", key=f"add_{agent_type}", use_container_width=True):
                            st.session_state.selected_crew_agents.append(
                                {"type": agent_type, "name": agent_info.get("name", agent_type)}
                            )
                            save_selected_agents(st.session_state.selected_crew_agents)
                            st.rerun()
                    else:
    ...
    with col1:
        st.button(
            "💾 Salvar Rascunho",
            key="save_draft",
            help="Guarda sua seleção para continuar depois",
    ...
    with col2:
        st.button(
            "🗑️ Limpar Seleção",
            key="clear_selection",
            help="Remove todos os agentes selecionados",
            use_container_width=True,
        )
    with col3:
    ...
    if submit_button:
        ...
        selected_tasks = [
            agent_to_task_mapping.get(agent_type, "research_task") for agent_type in selected_agents_types
        ]
            ...
            try:
                ...
                if crew:
                    ...
                    with st.expander("📋 Resumo da Crew Criada", expanded=True):
                        ...
                        st.markdown("---")
                        st.markdown(
                            "**💡 Como funciona:** Os agentes trabalham em sequência, passando informações entre si até completar a tarefa."
                        )
